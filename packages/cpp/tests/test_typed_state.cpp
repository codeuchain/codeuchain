...
void test_basic_typed_operations() {
    ...
    // Create typed state
    auto ctx = make_typed_state<std::string>(State{});
...
void test_type_evolution() {
    ...
    // Original data should still be accessible via base state
    auto base_ctx = ctx3.to_state();
...
void test_runtime_compatibility() {
    ...
    // Access via base state
    auto base_ctx = ctx2.to_state();
...
void test_state_operations() {
    ...
    assert(!ctx3.empty() && "State should not be empty");
...
int main() {
    std::cout << "CodeUChain Typed State Tests" << std::endl;
    std::cout << "===============================" << std::endl;
